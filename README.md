...
Print `marker/jabra-marker.png` — a clean **AR card** containing only two things: a scannable **QR code** at the top and the bold **J+** tracking marker below it. There is **no heading, no instruction text, and no step labels** on the card. The two-step flow is still:

1. **Scan to open** — the QR code at the top encodes the AR web page (<https://mrsdnf.github.io/WebAr/>). The user opens their phone camera and scans it to load the page (no app install).
2. **Aim at the J+** — once the page is open, the user aims the camera at the large, bold **"J+"** tracking marker below the QR to view the content in 3D.

The J+ is kept deliberately simple so it tracks reliably (the old multi-line text didn't track). Print the **whole card** on white paper or card, sized so the marker square is roughly **5–8 cm**. Keep the marker square's solid **black border fully intact** with a white margin around it — do not crop it; the border is what the tracker locks onto. The QR code above the square does not affect tracking. That printed sheet is "the card."
